...
type OldValue<WatchOptions, V> = WatchOptions extends { immediate: true }
    ? V | undefined
    : V;

export function Watch<Key extends string, Opt extends WatchOptions>(
    watchName: Key,
    options?: Opt
) {
    return function Watch<T extends Record<Key, any>>(
        ...
        descriptor: TypedPropertyDescriptor<
            (value: T[Key], oldValue: OldValue<Opt, T[Key]>) => any
        >
    ) {
        PassOnTo(() => {
